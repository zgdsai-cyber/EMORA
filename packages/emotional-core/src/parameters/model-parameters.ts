...
import type { DynamicsParameterSet, ModelParameters } from '../domain/model-parameters';
...
const emptyEmotionRecord = () =>
  Object.freeze(
    Object.fromEntries(emotionNames.map((emotion) => [emotion, 0])) as Record<EmotionName, number>,
  );
...
export function createDefaultModelParameters(): DeterministicModelParameters {
  ...
  return Object.freeze({
    ...
    dynamics: Object.freeze({
      eventImpactWeights: Object.freeze({ surpriseBase: 0.5, surpriseScale: 0.5 }),
      ...
      interactionWeights: Object.freeze({
        ...interactionWeights,
        fear: Object.freeze({ ...interactionWeights.fear, trust: -0.35, anger: 0.2 }),
...
function validateFinite(value: number, name: string, minimum?: number, maximum?: number) {
  if (!Number.isFinite(value) || (minimum !== undefined && value < minimum) || (maximum !== undefined && value > maximum)) {
    throw new InvalidDomainObjectError(`Model parameter ${name} must be a valid finite value.`);
  }
}

export function validateModelParameters(input: ModelParameters): DeterministicModelParameters {
  ...
  const configuredTrustWeights = dynamics.emotionWeights.trust as unknown as Record<string, number>;
  if (
  ...
  validateFinite(dynamics.eventImpactWeights.surpriseBase, 'eventImpactWeights.surpriseBase', 0, 1);
  validateFinite(dynamics.eventImpactWeights.surpriseScale, 'eventImpactWeights.surpriseScale', 0, 1);
  ...
  for (const emotion of emotionNames) {
    for (const [name, value] of Object.entries(dynamics.emotionWeights[emotion])) {
      validateFinite(value, `emotionWeights.${emotion}.${name}`, 0, 2);
    }
    validateFinite(dynamics.memoryWeights[emotion], `memoryWeights.${emotion}`, 0, 1);
    validateFinite(dynamics.stabilityWeights.baseline[emotion], `stabilityWeights.baseline.${emotion}`, 0, 1);
    for (const [target, value] of Object.entries(dynamics.interactionWeights[emotion])) {
  ...
  validateFinite(dynamics.confidenceWeights.base, 'confidenceWeights.base', 0, 1);
  validateFinite(dynamics.confidenceWeights.uncertaintyPenalty, 'confidenceWeights.uncertaintyPenalty', 0, 1);
  validateFinite(dynamics.confidenceWeights.memoryWithData, 'confidenceWeights.memoryWithData', 0, 1);
  validateFinite(dynamics.confidenceWeights.memoryWithoutData, 'confidenceWeights.memoryWithoutData', 0, 1);
