...
integration('Better Auth authentication', () => {
  afterAll(async () => {
    if (database) {
      for (const email of createdEmails) {
        await database.delete(schema.users).where(eq(schema.users.email, email));
      }
    }
  ...
  it('registers, retrieves a session, logs out, and logs in', async () => {
    ...
    const registerResponse = await auth.handler(new Request('http://localhost:3000/api/auth/sign-up/email', {
      method: 'POST',
    ...
    const logoutResponse = await auth.handler(new Request('http://localhost:3000/api/auth/sign-out', {
      method: 'POST',
    ...
    const loginResponse = await auth.handler(new Request('http://localhost:3000/api/auth/sign-in/email', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
